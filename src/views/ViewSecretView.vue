        ...
        <div class="column is-full-mobile is-two-thirds-tablet is-half-desktop is-one-third-widescreen">
          <div class="box secret-display-box animated-entry" :class="{'is-processing': isDecrypting}">
            ...
            <div class="view-content-wrapper">
              ...
              <div v-else-if="!encryptedPayloadFromServer && !decryptedMessage && !isDecrypting" class="notification is-warning is-light custom-notification">
                <p>链接中未找到有效加密数据或数据已失效。</p>
                <router-link to="/" class="button is-primary is-light mt-4 is-fullwidth">返回创建</router-link>
              </div>

              <div v-else-if="!decryptedMessage" class="password-prompt-panel">
...
const renderedMarkdown = computed(() => markdownService.render(decryptedMessage.value));
...
async function handleDecrypt() {
  ...
  try {
    decryptedMessage.value = await cryptoService.decrypt(encryptedPayloadFromServer.value, actualPasswordToTry);
  } catch (e) {
